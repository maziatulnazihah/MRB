...
<html lang="en">
	...
	<style>
			...
			header{
			background-image:linear-gradient(rgba(0,0,0,0.5),rgba(0,0,0,0.5)),url(269.jpg);
			height: auto;
		...
		<table border= "1px">
				...
				$sql = "SELECT * FROM mcr WHERE categories = 'breakfast'"; //tukar kt categories kalau nk lunch, dinner, etc...
				...
				if(mysqli_num_rows($result)>0){
					...
					while($row = mysqli_fetch_assoc($result)){
                        ...
                        $image_src = "image/".$image;
						 echo "<tr><td><img src=".$image_src." width='100' height='100' ></td>";
						echo "<td>" . $row["title"]."</td><td>" .$row["duration"]. "</td><td>". $row["serving"]."</td><td>";
						 echo"<a href=\"deletebf.php?ID=".$row["r_id"]."\">Edit</a></br>";
						 echo"<a href=\"bf1.php?ID=".$row["r_id"]."\">Read More</a></td></tr>";
